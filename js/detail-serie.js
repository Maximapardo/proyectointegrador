...
function recommendations() {
  ...
  console.log(Idtv);
  const url = 'https://api.themoviedb.org/3/tv/' + Idtv + '/recommendations?language=en-US&page=1';
  const options = {
    method: 'GET',
    headers: {
      accept: 'application/json',
  ...
  fetch(url, options)
    ...
    .then(json => {
      ...
      recomendacionesSeries.innerHTML = ''; // Limpiar contenido antes de agregar nuevas recomendaciones
      ...
      for (let i = 0; i < 1; i++) {
        ...
        <h4 class="fechas-reco">${json.results[i].name}</h4>
        </div>`;
      }
    })
    .catch(err => console.error('error:' + err));
